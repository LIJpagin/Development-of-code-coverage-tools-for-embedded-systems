...
bool CCameraBase::IsPilotMenuAvailable() const {COV_LOG_FUNC(1701);
    COV_LOG_FUNC(1698);
    return _state == stDisabled;
}

void CCameraBase::WriteIinitSequence() {COV_LOG_FUNC(1701);
    COV_LOG_FUNC(1698);
    ...
    for (int i = 0; i < INIT_SEQUENCE_LENGTH; i++) {COV_LOG_FOR(1701);
        COV_LOG_FOR(1698);
...
CVideo::STANDARD CCameraBase::GetStandard() {COV_LOG_FUNC(1701);
    COV_LOG_FUNC(1698);
    BYTE data = 0;
    _i2cDriver.Read(0x10, &data, 1);
    data &= 0x70;
    if ((data == 0x60) || (data == 0x40) || (data == 0x30) || (data == 0x20)) {COV_LOG_IF(1701);
        COV_LOG_IF(1698);
        return CVideo::pal;
    } else if ((data == 0x10) || (data == 0x00)) {COV_LOG_IF(1701);
        COV_LOG_IF(1698);
        return CVideo::ntsc;
    } else if (data == 0x50) {COV_LOG_IF(1701);
        COV_LOG_IF(1698);
        return CVideo::secam;
    } else {COV_LOG_ELSE(1701);
        COV_LOG_ELSE(1698);
        return CVideo::pal;  // заглушка, попадать сюда не должен.
    }
}

void CCameraBase::ManualConfigSynchro() {COV_LOG_FUNC(1701);
    COV_LOG_FUNC(1698);
...
void CCameraBase::LoadConfig() {COV_LOG_FUNC(1701);
    COV_LOG_FUNC(1698);
    _videoDriver->Reset();
    WriteIinitSequence();
    ...
    if (_loadingConfigType == lcManual) {COV_LOG_IF(1701);
        COV_LOG_IF(1698);
        ManualConfigSynchro();
    }
    _videoDriver->TurnOn();
...
void CCameraBase::RefreshConfig() {COV_LOG_FUNC(1701);
    COV_LOG_FUNC(1698);
