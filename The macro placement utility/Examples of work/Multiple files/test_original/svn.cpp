...
SVN::InformationAboutSystem::InformationAboutSystem(IUsingSetupMenu* context, const char*& name,
	SetupMenu::ACCESS_LEVEL AccessLevel, int id, UINT funcType) : Item(context, name, AccessLevel, id, funcType)
{
...
void SVN::ArincWord::SendingToSvn::SetDisplayedCamera(const SVN::Camera camera)
{
	switch (camera)
	{
		case Camera::None:		videoSourceCode = 0b000;	break;
		case Camera::TK4Adeg10:	videoSourceCode = 0b001;	break;
		case Camera::TK3A:		videoSourceCode = 0b010;	break;
		case Camera::TK4Adeg30:	videoSourceCode = 0b011;	break;
		case Camera::TK2A:		videoSourceCode = 0b100;	break;
		case Camera::TK4Adeg20:	videoSourceCode = 0b101;	break;
		case Camera::TK4Adeg0:	videoSourceCode = 0b110;	break;
		case Camera::TK4Adeg40:	videoSourceCode = 0b111;	break;
	}
}
...
SVN::Camera SVN::ArincWord::ReceptionToSvn::GetDisplayedCamera() const
{
	...
	{
		case 0b000: return Camera::None;
		case 0b001: return Camera::TK4Adeg10;
		case 0b010: return Camera::TK3A;
		case 0b011: return Camera::TK4Adeg30;
		case 0b100: return Camera::TK2A;
		case 0b101: return Camera::TK4Adeg20;
		case 0b110: return Camera::TK4Adeg0;
		case 0b111: return Camera::TK4Adeg40;
	}
}
...
SVN::State SVN::ArincWord::ReceptionToSvn::GetDeviceState(const Device device) const
{
	switch (device)
	{
		case Device::BUK1AB: return isSvnServiceability == 1 ? State::Normal : State::Denial;
		case Device::TK2A: return isVideoFromTK2A == 1 ? State::Normal : State::Denial;
		case Device::TK3A: return isVideoFromTK3A == 1 ? State::Normal : State::Denial;
		case Device::TK4A: return isVideoFromTK4A == 1 ? State::Normal : State::Denial;
		default: return State::Denial;
	}
}
...
SVN::SVN(CVideo* videoDriver, CArinc429In* arinc429in, CArinc429Out* arinc429out)
	: CCameraBase(videoDriver), _arinc429in(arinc429in), _arinc429out(arinc429out),
	_cameraSettings(this, lang_SVN_CAMERA_SETTINGS, SetupMenu::aclSystem, SetupMenu::idNull, setEnter | setExit, &_i2cDriver, _videoDriver),
	_systemState(lang_SYSTEM_STATE, this, pmSvnSystemState)
{
	InitSetupMenu();
	InitLanguageVariables();
...
{
	if (GetState() == stDisabled 
		&& (paramState[svnNoData] == mxFail
		|| paramState[svnFail] == mxFail
		|| paramState[svnFailCtrl] == mxFail
		|| _deviceStates[Device::BUK1AB] == State::Denial))
	{
		buff = lang_NAME;
		color = clYellow;
		return true;
...
void SVN::DrawCamerasIcons() const
{
	...
	{
		case Camera::TK2A: selectedDevice = Device::TK2A; break;
		case Camera::TK3A: selectedDevice = Device::TK3A; break;
		case Camera::TK4Adeg0:
		case Camera::TK4Adeg10:
		case Camera::TK4Adeg20:
		case Camera::TK4Adeg30:
		case Camera::TK4Adeg40: selectedDevice = Device::TK4A; break;
		case Camera::None: selectedDevice = 0; break;
	}
	int showDevice = 0;
	// Какая иконка камеры будет выделена при отображении СВН
	switch (_showCamera)
	{
		case Camera::TK2A: showDevice = Device::TK2A; break;
		case Camera::TK3A: showDevice = Device::TK3A; break;
		case Camera::TK4Adeg0:
		case Camera::TK4Adeg10:
		case Camera::TK4Adeg20:
		case Camera::TK4Adeg30:
		case Camera::TK4Adeg40: showDevice = Device::TK4A; break;
		case Camera::None: showDevice = 0; break;
	}
	...
	Camera showDegrees = (selectedDevice != Device::TK4A
			&& _selectedDegreesTK4A != _showCamera
			&& showDevice == Device::TK4A) ? _showCamera : _selectedDegreesTK4A;
	switch (showDegrees)
	{
		case Camera::TK4Adeg0:	degreesStr = "0°";	break;
		case Camera::TK4Adeg10: degreesStr = "10°";	break;
		case Camera::TK4Adeg20: degreesStr = "20°";	break;
		case Camera::TK4Adeg30: degreesStr = "30°";	break;
		case Camera::TK4Adeg40: degreesStr = "40°";	break;
		default: degreesStr = "";
	}
				...
				case ADDRESS_OF_DATA_RECEPTION_SVN:
					{
						...
						if (!_isStateWordReceived)
						{
							...
							if (_showCamera != Camera::None) {
								...
								if (_showCamera != Camera::TK2A
									|| _showCamera != Camera::TK3A)
								_selectedDegreesTK4A = _showCamera;
							}
							else
								_selectedCamera = Camera::TK2A;
...
SVN::MatrixState SVN::GetMatrixState(const UINT matrix) const
{
	switch (matrix)
	{
		case 0b00: return MatrixState::Normal;
		case 0b10: return MatrixState::Unreliable;
		case 0b01: return MatrixState::NotUsed;
		case 0b11: return MatrixState::Failure;
	}
}
